...
class GlobalExceptionHandlerTest {
    ...
    void shouldHandleBaseException() {
        ...
        assertThat(response.getBody().getProperties()).containsEntry("errorCode", "USER_NOT_FOUND");
        assertThat(response.getBody().getType().toString()).isEqualTo("https://telcocrm.com/errors/user-not-found");
        assertThat(response.getBody().getDetail()).contains(userId.toString());
    }
    ...
    void shouldHandleValidationException() {
        ...
        bindingResult.addError(new FieldError("target", "username", "Username must not be blank"));
        ...
        assertThat(errors.get("username")).contains("Username must not be blank");
    }
    ...
    void shouldHandleTypeMismatchException() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().getDetail()).contains("id").contains("not-a-uuid");
    }
    ...
    void shouldHandleOptimisticLockingFailureException() {
        var ex = new ObjectOptimisticLockingFailureException("User", UUID.randomUUID());
    ...
    void shouldHandleIllegalStateException() {
        var ex = new IllegalStateException("Role already exists with name: SYSTEM_ADMIN");
        ...
        assertThat(response.getBody().getDetail()).isEqualTo("Role already exists with name: SYSTEM_ADMIN");
    }

    @Test
    void shouldHandleIllegalArgumentException() {
        var ex = new IllegalArgumentException("Invalid request");

        ResponseEntity<ProblemDetail> response = handler.handleIllegalArgumentException(ex);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().getTitle()).isEqualTo("INVALID_REQUEST");
    }

    private Request dummyRequest() {
        return Request.create(Request.HttpMethod.GET, "/admin/realms/telcocrm-gygy5/roles/FIELD_DEALER", Collections.emptyMap(), null, StandardCharsets.UTF_8);
    }
    ...
    void shouldHandleOtherFeignExceptionAs503() {
        FeignException ex = new FeignException(403, "forbidden") {};

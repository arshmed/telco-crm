...
class GlobalExceptionHandlerTest {
    ...
    void shouldHandleBaseException() {
        ...
        var ex = new PaymentNotFoundException(paymentId);

        ResponseEntity<ProblemDetail> response = handler.handleBaseException(ex);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody().getTitle()).isEqualTo("PAYMENT_NOT_FOUND");
        assertThat(response.getBody().getProperties()).containsEntry("errorCode", "PAYMENT_NOT_FOUND");
        assertThat(response.getBody().getDetail()).contains(paymentId.toString());
    }
    ...
    void shouldHandleValidationException() {
        ...
        var errors = (Map<String, List<String>>) response.getBody().getProperties().get("errors");
        assertThat(errors.get("amount")).contains("Amount must be positive");
    }
    ...
    void shouldHandleOptimisticLockingFailureException() {
        var ex = new ObjectOptimisticLockingFailureException("Payment", UUID.randomUUID());
    ...
    void shouldHandleIllegalStateException() {
        var ex = new IllegalStateException("Payment cannot be refunded twice");
        ...
        assertThat(response.getBody().getDetail()).isEqualTo("Payment cannot be refunded twice");
    }

    @Test
    void shouldHandleIllegalArgumentException() {
        var ex = new IllegalArgumentException("Invalid currency code");
